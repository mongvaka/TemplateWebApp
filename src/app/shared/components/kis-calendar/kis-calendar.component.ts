...
export class KisCalendarComponent extends KisBaseComponent implements OnInit {
  //#region variable
  bindingModel: any;
  ...
  @Input() set kisConfig(param: CalendarConfigModel) {
  ...
  onModelChange(kisModel: any) {
    this.bindingTimeOnly
      ? this.kisModelChange.next(
          `${kisModel.getHours()}:${kisModel.getMinutes()}:`
        )
      : this.kisModelChange.next(kisModel);
  }
}
