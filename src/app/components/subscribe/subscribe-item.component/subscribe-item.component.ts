...
import { SubscribeItemCustomComponent } from './subscribe-item-custom.component';
import { ROUTE_INFO_GEN } from 'app/shared/constants';
@Component({
  selector: 'app-subscribe-item',
  templateUrl: './subscribe-item.component.html',
  styleUrls: ['./subscribe-item.component.scss'],
})
export class SubscribeItemComponent
  extends BaseItemComponent<SubscribeItemModel>
  implements BaseItemInterface
{
  ...
  constructor(
    ...
    public custom: SubscribeItemCustomComponent
  ) {
    super();
    ...
    this.infoItems = [
      {
        ...
        command: () => this.toInfo({ path: ROUTE_INFO_GEN.SUBSCRIBE_TRANS }),
      },
    ];
 ...
 // tslint:disable-next-line: use-lifecycle-interface
  ...
  getById(): Observable<SubscribeItemModel> {
    return this.service.getSubscribeTableById(this.id);
  }
  ...
  onSubmit(isColsing: boolean): void {
    if (this.isUpdateMode) {
      super.onUpdate(this.service.editSubscribeTable(this.model), isColsing);
    } else {
      super.onCreate(this.service.createSubscribeTable(this.model), isColsing);
    }
  }
