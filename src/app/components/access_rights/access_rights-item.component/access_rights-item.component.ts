...
import { AccessRightsItemCustomComponent } from './access_rights-item-custom.component';
@Component({
  selector: 'app-access-rights-item',
  templateUrl: './access_rights-item.component.html',
  styleUrls: ['./access_rights-item.component.scss'],
})
export class AccessRightsItemComponent
  extends BaseItemComponent<AccessRightsItemModel>
  implements BaseItemInterface
{
  ...
  constructor(
    ...
    public custom: AccessRightsItemCustomComponent
  ) {
    super();
 ...
 // tslint:disable-next-line: use-lifecycle-interface
  ...
  getById(): Observable<AccessRightsItemModel> {
    return this.service.getAccessRightsTableById(this.id);
  }
  ...
  onAsyncRunner(model?: any): void {
    forkJoin(this.baseDropdown.getRoleDropDown()).subscribe(
      ([role]) => {
  ...
  onSubmit(isColsing: boolean): void {
    if (this.isUpdateMode) {
      super.onUpdate(this.service.editAccessRightsTable(this.model), isColsing);
    } else {
      super.onCreate(this.service.createAccessRightsTable(this.model), isColsing);
    }
  }
