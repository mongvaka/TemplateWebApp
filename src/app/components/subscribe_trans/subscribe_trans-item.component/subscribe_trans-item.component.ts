...
import { SubscribeTransItemCustomComponent } from './subscribe_trans-item-custom.component';
import { UIControllerService } from 'app/core/services/uiController.service';
@Component({
  selector: 'app-subscribe-trans-item',
  templateUrl: './subscribe_trans-item.component.html',
  styleUrls: ['./subscribe_trans-item.component.scss'],
})
export class SubscribeTransItemComponent
  extends BaseItemComponent<SubscribeTransItemModel>
  implements BaseItemInterface
{
  ...
  featuretableOptions: SelectItems[] = [];
  constructor(
    private service: SubscribeTransService,
    private currentActivatedRoute: ActivatedRoute,
    public custom: SubscribeTransItemCustomComponent,
 ...
 // tslint:disable-next-line: use-lifecycle-interface
  ...
  getById(): Observable<SubscribeTransItemModel> {
    return this.service.getSubscribeTransTableById(this.id);
  }
  getInitialData(): Observable<SubscribeTransItemModel> {
  ...
  onAsyncRunner(model?: any): void {
    forkJoin(
      ...
      this.baseDropdown.getFeatureDropDown()
      ).subscribe(
      ([
        subscribetable,
        featuretable
        ...
        this.featuretableOptions = featuretable;
  ...
  setFieldAccessing(): void {
    ...
    this.custom.getFieldAccessing(this.model, this.isRelatedMode).subscribe((res) => {
  ...
  onSubmit(isColsing: boolean): void {
    if (this.isUpdateMode) {
      super.onUpdate(this.service.editSubscribeTransTable(this.model), isColsing);
    } else {
      super.onCreate(this.service.createSubscribeTransTable(this.model), isColsing);
    }
  }
