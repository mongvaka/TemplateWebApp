...
import { RoleItemCustomComponent } from './role-item-custom.component';
...
export class RoleItemComponent
  extends BaseItemComponent<RoleItemModel>
  implements BaseItemInterface
{
  ...
  constructor(
    ...
    public custom: RoleItemCustomComponent
  ) {
    super();
  ...
  setInfoOptions(model?: any): void {
    this.infoItems = [
      {
        label: 'LABEL.ACCESS_FEATURE',
        command: () => this.toInfo({ path: ROUTE_INFO_GEN.ACCESS_FEATURE }),
      },
      // {
      //   label: 'LABEL.ASSESSMENT_TRANS',
      //   command: () => this.toInfo({ path: ROUTE_INFO_GEN.ASSESSMENT_TRANS }),
      // }
    ];
  ...
  onEnumLoader(): void {}
  getById(): Observable<RoleItemModel> {
    return this.service.getRoleTableById(this.id);
  }
  ...
  onSubmit(isColsing: boolean): void {
    if (this.isUpdateMode) {
      super.onUpdate(this.service.editRoleTable(this.model), isColsing);
    } else {
      super.onCreate(this.service.createRoleTable(this.model), isColsing);
    }
  }
